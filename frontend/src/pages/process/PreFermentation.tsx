...
export default function PreFermentation() {
 ...
 className="border px-1.5 py-0.5 text-sm" />
 <button onClick={() => { updateBatchTime(batchId, field, val); setEditing(false); }}
 className="text-xs bg-green-100 text-green-700 px-1.5 py-0.5">✓</button>
 <button onClick={() => { setVal(toLocal(value)); setEditing(false); }}
 className="text-xs bg-gray-100 text-gray-500 px-1.5 py-0.5">✕</button>
 </span>
 ) : (
 ...
 className="text-xs bg-blue-50 text-blue-600 px-1.5 py-0.5 hover:bg-blue-100">Set Now</button>
 )}
 </div>
 );
 };
 ...
 <div className={`px-3 py-1 text-xs font-medium ${i <= ci ? 'text-white' : 'text-gray-400 bg-gray-100'}`} style={i <= ci ? { backgroundColor: phaseColors[p] } : {}}>
 {phaseLabels[p]}
 ...
 <div><label className="text-xs text-gray-500">{label}</label><input type="number" step={step} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} className="w-full border px-2 py-1.5 text-sm" inputMode="decimal" /></div>
 );
 const txtField = (label: string, value: string, onChange: (v: string) => void, placeholder = '') => (
 <div><label className="text-xs text-gray-500">{label}</label><input value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} className="w-full border px-2 py-1.5 text-sm" /></div>
 );
 ...
 <button onClick={() => setShowNewBatch(true)} className="bg-white/20 hover:bg-white/30 px-4 py-2 text-sm font-medium flex items-center gap-1"><Plus size={16} /> New Batch</button>
 </div>
 </div>

 {/* NEW BATCH FORM */}
 {showNewBatch && (
 <div className="bg-white shadow p-4 border-l-4 border-indigo-500">
 ...
 className={`px-4 py-1.5 text-sm font-medium border ${nbForm.fermenterNo === String(n) ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-700 border-gray-300 hover:border-indigo-400'}`}>PF{n}</button>
 ))}
 </div>
 </div>
 ...
 <button onClick={createBatch} className="bg-indigo-600 text-white px-4 py-1.5 text-sm hover:bg-indigo-700">Create Batch</button>
 <button onClick={() => setShowNewBatch(false)} className="bg-gray-200 px-4 py-1.5 text-sm">Cancel</button>
 </div>
 </div>
 )}
 ...
 if (navigator.share) { navigator.share({ text: t }).catch(() => { window.open(`https://t.me/share/url?text=${encodeURIComponent(t)}`, '_blank'); }); } else { window.open(`https://t.me/share/url?text=${encodeURIComponent(t)}`, '_blank'); }
 }} className="text-white/80 hover:text-white p-1.5 hover:bg-white/10 transition" title="Share on Telegram">
 <Share2 size={18} />
 </button>
 <span className="px-3 py-1 text-sm font-bold text-white" style={{ backgroundColor: phaseColors[activeBatch.phase] }}>{phaseLabels[activeBatch.phase]}</span>
 </div>
 </div>
 <PhaseTimeline batch={activeBatch} />
 {phaseMsg && (
 <div className="mx-4 px-3 py-2 bg-green-50 border border-green-200 text-green-700 text-sm font-medium flex items-center gap-2 animate-pulse">
 {phaseMsg}
 </div>
 )}
 ...
 <div className="mb-3 bg-white border border-amber-300 p-3">
 <div className="flex items-center justify-between mb-2">
 ...
 className="bg-amber-500 text-white px-3 py-1.5 text-sm hover:bg-amber-600 font-medium disabled:opacity-50">
 {applyingRecipe ? 'Applying...' : 'Apply All'}
 </button>
 </div>
 <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
 {pfRecipes.map(r => (
 <div key={r.id} className="flex justify-between bg-amber-50 px-2 py-1 text-sm">
 ...
 className="w-20 border px-1.5 py-0.5 text-sm text-right" autoFocus
 ...
 <span className="cursor-pointer hover:bg-amber-100 px-1" onClick={() => { setEditingDoseId(d.id); setEditDoseQty(String(d.quantity)); }}>
 ...
 className={`px-3 py-1.5 text-sm border transition-colors ${doseForm.chemicalName === c.name ? 'bg-amber-500 text-white border-amber-500 font-medium' : 'bg-white text-gray-700 border-gray-300 hover:border-amber-400 hover:bg-amber-50'}`}>
 {c.name}
 </button>
 ))}
 {!isField && <button onClick={() => setShowNewChem(true)} className="px-3 py-1.5 text-sm border border-dashed border-amber-400 text-amber-600 hover:bg-amber-50">+ New</button>}
 </div>
 <div className="flex gap-2 items-end flex-wrap">
 {numField('Quantity', doseForm.quantity, v => setDoseForm(f => ({ ...f, quantity: v })), '0.1')}
 <div>
 <label className="text-xs text-gray-500">Unit</label>
 <select value={doseForm.unit} onChange={e => setDoseForm(f => ({ ...f, unit: e.target.value }))} className="border px-2 py-1.5 text-sm">
 <option value="kg">kg</option><option value="ltr">ltr</option><option value="gm">gm</option><option value="ml">ml</option>
 </select>
 </div>
 <button onClick={addDosing} className="bg-amber-500 text-white px-3 py-1.5 text-sm hover:bg-amber-600 mb-0.5">Add</button>
 </div>
 {activeBatch.dosings.length > 0 && (
 <div className="mt-3 pt-3 border-t border-amber-200">
 <button onClick={() => {
 advancePhase(activeBatch, 'LAB', { dosingEndTime: new Date().toISOString() });
 }} className="bg-emerald-600 text-white px-4 py-2 text-sm hover:bg-emerald-700 font-medium">Finish Dosing → Lab</button>
 </div>
 )}
 </div>
 {showNewChem && !isField && (
 <div className="mt-2 flex gap-2 items-end bg-white p-2 border">
 {txtField('Chemical Name', chemForm.name, v => setChemForm(f => ({ ...f, name: v })), 'e.g. Urea')}
 <div>
 <label className="text-xs text-gray-500">Unit</label>
 <select value={chemForm.unit} onChange={e => setChemForm(f => ({ ...f, unit: e.target.value }))} className="border px-2 py-1.5 text-sm">
 <option value="kg">kg</option><option value="ltr">ltr</option><option value="gm">gm</option>
 </select>
 </div>
 <button onClick={addChemical} className="bg-green-600 text-white px-3 py-1.5 text-sm">Save</button>
 <button onClick={() => setShowNewChem(false)} className="bg-gray-200 px-3 py-1.5 text-sm">×</button>
 </div>
 )}
 </div>
 )}
 ...
 <>
 {!isField && <button onClick={() => advancePhase(activeBatch, 'DOSING')} className="bg-amber-500 text-white px-4 py-2 text-sm hover:bg-amber-600 flex items-center gap-1">← Back to Dosing</button>}
 <button onClick={() => startTransfer(activeBatch)}
 className="bg-blue-600 text-white px-4 py-2 text-sm hover:bg-blue-700 flex items-center gap-1"><ArrowRight size={16} /> Transfer to Fermenter</button>
 </>
 )}
 ...
 }} className="bg-purple-600 text-white px-4 py-2 text-sm hover:bg-purple-700 flex items-center gap-1"><RotateCcw size={16} /> Start CIP</button>
 </>
 )}
 ...
 }} className="bg-gray-600 text-white px-4 py-2 text-sm hover:bg-gray-700">End CIP → Done</button>
 </>
 )}
 </div>
 </div>
 ...
 <div className="bg-white md: w-full max-w-md p-5 shadow-2xl" onClick={e => e.stopPropagation()}>
 ...
 className={`py-3 text-lg font-bold border-2 transition ${transferTarget.fermenterNo === String(n) ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-700 border-gray-200 hover:border-blue-400'}`}>
 F{n}
 </button>
 ))}
 </div>
 ...
 className="flex-1 bg-blue-600 text-white py-3 text-base font-semibold hover:bg-blue-700 flex items-center justify-center gap-2">
 <ArrowRight size={18} /> Transfer & Start Filling
 </button>
 <button onClick={() => setShowTransferModal(false)}
 className="px-6 py-3 bg-gray-100 text-gray-600 text-base font-medium hover:bg-gray-200">Cancel</button>
 </div>
 </div>
 </div>
 )}
 ...
 <span className="px-2 py-0.5 text-xs font-medium text-white" style={{ backgroundColor: phaseColors[b.phase] }}>{phaseLabels[b.phase]}</span>
 ...
 <button onClick={e => { e.stopPropagation(); setActiveBatchId(b.id); }} className="text-xs bg-indigo-100 text-indigo-700 px-2 py-1 hover:bg-indigo-200">Select</button>
 )}
 ...
 <span key={d.id} className="text-xs bg-amber-100 text-amber-800 px-2 py-1">
