...
export default function Fermentation() {
 ...
 className="p-1.5 bg-green-600 hover:bg-green-500 active:bg-green-700 transition-colors">
 <MessageCircle size={16} />
 </button>
 <button onClick={() => { setLoading(true); load(); }} className="p-1.5 hover:bg-white/10 active:bg-white/20 transition-colors"><RefreshCw size={16} /></button>
 </div>
 </div>
 </div>

 {msg && (
 <div className={`mx-3 mt-2 p-2 text-xs flex items-center gap-1.5 ${msg.t === 'ok' ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'}`}>
 ...
 <div className="bg-gradient-to-br from-indigo-500 to-purple-600 text-white p-3 col-span-1">
 ...
 <div className="bg-white border border-gray-200 p-3 col-span-1">
 ...
 return (
 <span key={vw.label} className="text-[7px] font-bold text-indigo-500 bg-indigo-50 px-1 py-0.5">
 ...
 className={`relative p-2.5 text-left transition-all duration-200 border-2 ${
 isSelected ? `${cfg.bg} ring-2 ${cfg.ring} border-transparent scale-[1.03]`
 : isIdle ? 'bg-white border-gray-100 hover:border-gray-300 hover:'
 : `bg-white border-gray-100 hover:hover:border-gray-200`
 }`}>
 ...
 <span className={`text-[10px] font-extrabold px-1.5 py-0.5 ${cfg.bg} ${cfg.text}`}>{cfg.label}</span>
 ...
 {levelStr && <span className="text-xs font-extrabold text-blue-800 bg-blue-100 px-1.5 py-0.5">{levelStr}</span>}
 </div>
 )}
 ...
 className="absolute bottom-1.5 right-1.5 p-1 bg-green-100 text-green-700 hover:bg-green-200 active:bg-green-300 transition-colors"
 ...
 return (
 <div className="mx-3 mt-3 bg-white border border-gray-200 overflow-hidden animate-[slideUp_0.2s_ease-out]">
 ...
 <div className={`w-8 h-8 flex items-center justify-center ${isBW ? 'bg-amber-200' : cfg.bg}`}>
 ...
 <button onClick={() => setSelected(null)} className="ml-auto w-7 h-7 bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-colors"><X size={14} className="text-gray-500" /></button>
 </div>
 ...
 className="px-4 py-2 bg-indigo-600 text-white text-sm font-semibold hover:bg-indigo-700 flex items-center gap-1.5 mx-auto">
 ...
 <input type="number" value={newBatchForm.batchNo} onChange={e => setNewBatchForm(f => ({ ...f, batchNo: e.target.value }))} className="w-full px-2 py-2 border border-gray-300 text-base font-bold" /></div>
 <div><label className="text-[10px] font-bold text-gray-600">Level %</label>
 <input type="number" step="0.1" value={newBatchForm.pfLevel} onChange={e => setNewBatchForm(f => ({ ...f, pfLevel: e.target.value }))} className="w-full px-2 py-2 border border-gray-300 text-base font-bold" /></div>
 </div>
 <div className="grid grid-cols-2 gap-2">
 <div><label className="text-[10px] font-bold text-gray-600">Gravity</label>
 <input type="number" step="0.001" value={newBatchForm.slurryGravity} onChange={e => setNewBatchForm(f => ({ ...f, slurryGravity: e.target.value }))} className="w-full px-2 py-2 border border-gray-300 text-base font-bold" /></div>
 {isPF && <div><label className="text-[10px] font-bold text-gray-600">Temp °C</label>
 <input type="number" step="0.1" value={newBatchForm.slurryTemp} onChange={e => setNewBatchForm(f => ({ ...f, slurryTemp: e.target.value }))} className="w-full px-2 py-2 border border-gray-300 text-base font-bold" /></div>}
 </div>
 <div className="flex gap-2">
 <button onClick={startBatch} disabled={saving} className="flex-1 py-2 bg-indigo-600 text-white text-xs font-bold disabled:opacity-50">
 {saving ? <Loader2 size={14} className="animate-spin mx-auto" /> : 'Start'}
 </button>
 <button onClick={() => setShowNewBatch(false)} className="px-3 py-2 bg-gray-100 text-gray-600 text-xs">Cancel</button>
 </div>
 </div>
 )}
 </div>
 ...
 }} className={`flex-1 text-center py-1.5 text-[9px] font-extrabold ${
 ...
 {isPF && (batch as PFBatch).slurryVolume && <div className="bg-indigo-100 px-2.5 py-1 text-[11px]"><span className="text-indigo-500 font-bold">Vol</span> <span className="font-extrabold text-indigo-800">{((batch as PFBatch).slurryVolume! / 1000).toFixed(0)} KL</span></div>}
 {isPF && (batch as PFBatch).slurryGravity && <div className="bg-indigo-100 px-2.5 py-1 text-[11px]"><span className="text-indigo-500 font-bold">SG</span> <span className="font-extrabold text-indigo-800">{(batch as PFBatch).slurryGravity!.toFixed(3)}</span></div>}
 {isPF && (batch as PFBatch).slurryTemp && <div className="bg-indigo-100 px-2.5 py-1 text-[11px]"><span className="text-indigo-500 font-bold">Temp</span> <span className="font-extrabold text-indigo-800">{(batch as PFBatch).slurryTemp}°C</span></div>}
 {isFerm && (batch as FermBatch).fermLevel && <div className="bg-blue-100 px-2.5 py-1 text-[11px]"><span className="text-blue-500 font-bold">Lvl</span> <span className="font-extrabold text-blue-800">{(batch as FermBatch).fermLevel}%</span></div>}
 {isFerm && (batch as FermBatch).setupGravity && <div className="bg-indigo-100 px-2.5 py-1 text-[11px]"><span className="text-indigo-500 font-bold">SG</span> <span className="font-extrabold text-indigo-800">{(batch as FermBatch).setupGravity!.toFixed(3)}</span></div>}
 {isFerm && (batch as FermBatch).finalAlcohol && <div className="bg-emerald-100 px-2.5 py-1 text-[11px]"><span className="text-emerald-500 font-bold">Alc</span> <span className="font-extrabold text-emerald-800">{(batch as FermBatch).finalAlcohol}%</span></div>}
 {isFerm && (batch as FermBatch).retentionStartTime && phase === 'RETENTION' && <div className="bg-orange-100 px-2.5 py-1 text-[11px]"><span className="text-orange-500 font-bold">Ret</span> <span className="font-extrabold text-orange-800">{elapsed((batch as FermBatch).retentionStartTime)}</span></div>}
 {batch.remarks && <div className="bg-gray-100 px-2.5 py-1 text-[11px] text-gray-600 truncate max-w-[200px]" title={batch.remarks}>{batch.remarks}</div>}
 </div>
 ...
 className="px-3 py-1.5 bg-blue-600 text-white text-xs font-bold disabled:opacity-50">
 ...
 {isPF && phase === 'SETUP' && <button onClick={() => advancePhase('PF', batch!.id, 'DOSING')} disabled={saving} className="px-3 py-1.5 bg-violet-600 text-white text-xs font-bold disabled:opacity-50">→ Dosing</button>}
 {isPF && phase === 'DOSING' && <button onClick={() => advancePhase('PF', batch!.id, 'LAB')} disabled={saving} className="px-3 py-1.5 bg-cyan-600 text-white text-xs font-bold disabled:opacity-50">→ Lab</button>}
 {isPF && phase === 'CIP' && <button onClick={() => advancePhase('PF', batch!.id, 'DONE', { cipEndTime: new Date().toISOString() })} disabled={saving} className="px-3 py-1.5 bg-gray-600 text-white text-xs font-bold disabled:opacity-50">→ Complete</button>}

 {isFerm && phase === 'FILLING' && <button onClick={() => advancePhase('FERM', batch!.id, 'REACTION', { reactionStartTime: new Date().toISOString(), fillingEndTime: new Date().toISOString() })} disabled={saving} className="px-3 py-1.5 bg-amber-600 text-white text-xs font-bold disabled:opacity-50">→ Reaction</button>}
 {isFerm && phase === 'REACTION' && <button onClick={() => advancePhase('FERM', batch!.id, 'RETENTION', { retentionStartTime: new Date().toISOString() })} disabled={saving} className="px-3 py-1.5 bg-orange-600 text-white text-xs font-bold disabled:opacity-50">→ Retention</button>}
 ...
 setSaving(false);
 }} disabled={saving} className="px-3 py-1.5 bg-emerald-600 text-white text-xs font-bold disabled:opacity-50">→ Transfer & CIP</button>
 )}
 {isFerm && phase === 'CIP' && <button onClick={() => advancePhase('FERM', batch!.id, 'DONE', { cipEndTime: new Date().toISOString() })} disabled={saving} className="px-3 py-1.5 bg-gray-600 text-white text-xs font-bold disabled:opacity-50">→ Complete</button>}

 {/* Small delete */}
 <button onClick={deleteBatch}
 className="ml-auto px-2 py-1 text-[10px] text-red-400 hover:text-red-600 hover:bg-red-50 flex items-center gap-1">
 <Trash2 size={10} /> Del
 </button>
 </div>
 </div>
 ...
 className={`flex-1 py-1.5 text-[11px] font-bold tracking-wide transition-all duration-150 ${
 tab === t
 ...
 className="flex-1 px-2 py-1 text-xs font-semibold text-gray-900 border border-gray-200 bg-white focus:ring-2 focus:ring-indigo-300 outline-none" />
 </div>
 )}
 ...
 disabled={opcLoading}
 className="text-[8px] font-bold uppercase tracking-wider px-1.5 py-0.5 bg-green-100 text-green-700 border border-green-300 hover:bg-green-200 disabled:opacity-50"
 >
 {opcLoading ? '...' : 'OPC LIVE'}
 </button>
 )}
 ...
 className="w-full px-2 py-2 text-base font-bold text-gray-900 border border-gray-300 bg-white focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 outline-none transition-all" inputMode="decimal" />
 </div>
 ))}
 </div>
 <input placeholder="Remarks..." value={readingForm.remarks || ''} onChange={e => setReadingForm(f => ({ ...f, remarks: e.target.value }))}
 className="w-full px-2 py-1.5 text-xs border border-gray-200 bg-gray-50 outline-none" />
 <div className="flex gap-2.5">
 <button onClick={() => saveReading(false)} disabled={saving}
 className="flex-1 py-2.5 bg-indigo-600 text-white text-sm font-bold disabled:opacity-50 flex items-center justify-center gap-1.5 hover:bg-indigo-700 active:scale-[0.98] transition-all">
 {saving ? <Loader2 size={14} className="animate-spin" /> : <><Send size={13} /> Save</>}
 </button>
 <button onClick={() => saveReading(true)} disabled={saving}
 className="flex-1 py-2.5 bg-green-600 text-white text-sm font-bold disabled:opacity-50 flex items-center justify-center gap-1.5 hover:bg-green-700 active:scale-[0.98] transition-all">
 ...
 className="w-full flex items-center gap-2 px-2 py-1.5 bg-violet-50/60 mb-1 hover:bg-violet-100/60 transition-colors">
 ...
 <div key={d.id} className="flex items-center gap-2 bg-violet-50/50 px-2 py-1">
 ...
 className="w-14 px-1 py-0.5 text-[11px] border" autoFocus
 ...
 placeholder="Qty" className="w-full px-2 py-1.5 text-xs border" />
 </div>
 <div className="w-14">
 ...
 className="px-2.5 py-1.5 bg-violet-600 text-white text-xs font-bold disabled:opacity-50">
 <Plus size={13} />
 </button>
 </div>
 </div>
 ...
 <div className="flex items-center gap-2 px-2 py-1.5 bg-orange-50 border border-orange-200 mb-2"
 ...
 className="w-full py-1.5 mb-2 bg-violet-50 text-violet-700 border border-violet-200 text-xs font-bold hover:bg-violet-100 flex items-center justify-center gap-1">
 ...
 editingReading === r.id ? (
 /* Inline edit row */
 <div key={r.id} className="bg-yellow-50 p-2 border border-yellow-200 space-y-1.5">
 <div className="grid grid-cols-3 gap-1.5">
 {[
 ...
 className="w-full px-1 py-0.5 text-[10px] border" />
 </div>
 ))}
 </div>
 <div className="flex gap-1.5">
 <button onClick={() => saveEditReading(r.id)} className="flex-1 py-1 bg-green-600 text-white text-[10px] font-bold">Save</button>
 <button onClick={() => setEditingReading(null)} className="flex-1 py-1 bg-gray-200 text-gray-600 text-[10px] font-bold">Cancel</button>
 </div>
 </div>
 ) : (
 /* Normal row */
 <div key={r.id || i} className="flex items-center gap-2 px-2.5 py-2 bg-gray-50/80 text-xs group hover:bg-gray-100 transition-colors">
 ...
 {r.level != null && <span className="text-blue-800 font-extrabold bg-blue-100 px-1.5 py-0.5">Lvl {r.level}%</span>}
 {r.spGravity != null && <span className="text-indigo-800 font-extrabold bg-indigo-100 px-1.5 py-0.5">SG {typeof r.spGravity === 'number' ? r.spGravity.toFixed(3) : r.spGravity}</span>}
 {r.ph != null && <span className="text-gray-800 font-bold">pH {r.ph}</span>}
 {r.temp != null && <span className={`font-extrabold px-1.5 py-0.5 ${(r.temp || 0) > 37 ? 'text-red-800 bg-red-100' : 'text-orange-800 bg-orange-100'}`}>{r.temp}°C</span>}
 {r.alcohol != null && <span className="text-emerald-800 font-extrabold bg-emerald-100 px-1.5 py-0.5">Alc {r.alcohol}%</span>}
 {r.rs != null && <span className="text-amber-800 font-extrabold bg-amber-100 px-1.5 py-0.5">RS {r.rs}</span>}
 {r.rst != null && <span className="text-amber-700 font-bold bg-amber-100 px-1.5 py-0.5">RST {r.rst}</span>}
 {r.ds != null && <span className="text-purple-800 font-bold bg-purple-100 px-1.5 py-0.5">DS {r.ds}%</span>}
 {r.vfaPpa != null && <span className="text-rose-800 font-bold bg-rose-100 px-1.5 py-0.5">VFA {r.vfaPpa}</span>}
 {r.status === 'FIELD' && <span className="text-[10px] bg-yellow-200 text-yellow-900 px-1.5 py-0.5 font-bold">FIELD</span>}
 </div>
 {r.id && (
 <div className="flex items-center gap-1 shrink-0 opacity-40 group-hover:opacity-100 transition-opacity">
 <button onClick={() => startEditReading(r as any)} className="w-6 h-6 bg-blue-50 hover:bg-blue-100 flex items-center justify-center text-blue-600 transition-colors" title="Edit">
 ...
 <button onClick={() => deleteReading(r.id)} className="w-6 h-6 bg-red-50 hover:bg-red-100 flex items-center justify-center text-red-500 transition-colors" title="Delete">
 <Trash2 size={11} />
 </button>
 </div>
 )}
 ...
 <div className="bg-white border border-gray-200 shadow overflow-hidden">
 ...
 <div className="col-span-1"><span className={`px-1.5 py-0.5 text-[10px] font-bold ${pc.color}`}>{pc.label}</span></div>
 ...
 <input type="number" step="0.001" defaultValue={b.setupGravity || ''} className="w-20 px-1.5 py-0.5 border text-[10px] font-medium text-indigo-600"
 ...
 <input type="number" step="0.01" defaultValue={b.finalAlcohol || ''} className="w-20 px-1.5 py-0.5 border text-[10px] font-bold text-emerald-700"
 ...
 {b.yeast && <span className="bg-green-50 border border-green-100 px-2 py-0.5 text-[10px]"><span className="font-bold text-green-800">Yeast:</span> <span className="text-green-600">{b.yeast}</span></span>}
 {b.enzyme && <span className="bg-blue-50 border border-blue-100 px-2 py-0.5 text-[10px]"><span className="font-bold text-blue-800">Enzyme:</span> <span className="text-blue-600">{b.enzyme}</span></span>}
 {b.urea && <span className="bg-amber-50 border border-amber-100 px-2 py-0.5 text-[10px]"><span className="font-bold text-amber-800">Urea:</span> <span className="text-amber-600">{b.urea}</span></span>}
 {b.formolin && <span className="bg-red-50 border border-red-100 px-2 py-0.5 text-[10px]"><span className="font-bold text-red-800">Formolin:</span> <span className="text-red-600">{b.formolin}</span></span>}
 {b.booster && <span className="bg-purple-50 border border-purple-100 px-2 py-0.5 text-[10px]"><span className="font-bold text-purple-800">Booster:</span> <span className="text-purple-600">{b.booster}</span></span>}
 </div>
 )}
 </div>
 ...
 {dosings.map((d: any) => (
 <span key={d.id} className="bg-amber-50 border border-amber-100 px-2 py-0.5 text-[10px]">
