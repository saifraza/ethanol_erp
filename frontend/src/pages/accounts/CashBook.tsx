...
export default function CashBook() {
  ...
  const [showTip, setShowTip] = useState(() => !localStorage.getItem('cashbook_tip_dismissed'));
  ...
  const fetchAccounts = useCallback(async () => {
    try {
      ...
      const cash = res.data.find(a => a.code === '1001');
  ...
  const fetchLedger = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get<LedgerData>(`/journal-entries/ledger/${cashAccountId}`, { params });
      setLedger(res.data);
    } catch (err) {
  ...
  }, [cashAccountId, dateRange, divisionFilter]);
  ...
  // Cash is an ASSET — closing = opening + debit - credit
  const filteredClosing = ledger ? ledger.openingBalance + filtered.totalDebit - filtered.totalCredit : 0;
  ...
  useHotkeys([
    ...
    { key: 'n', handler: e => { if (cashAccountId) { e.preventDefault(); setNewEntryOpen(true); } } },
  ...
  const dismissTip = () => { localStorage.setItem('cashbook_tip_dismissed', '1'); setShowTip(false); };
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="p-3 md:p-6 space-y-0">
        ...
        <div className="bg-slate-800 text-white px-4 py-2.5 -mx-3 md:-mx-6 -mt-3 md:-mt-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] text-slate-400">Cash account register · keyboard-first · inline entry</span>
            {ledger && (
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => cashAccountId && setNewEntryOpen(true)}
        ...
        {loading && <div className="text-xs text-slate-400 uppercase tracking-widest py-4 px-4">Loading cash book...</div>}
          ...
          <>
            ...
            <div className="grid grid-cols-5 border-x border-b border-slate-300 -mx-3 md:-mx-6">
              ...
              <div className="bg-white px-4 py-3 border-r border-slate-300 border-l-4 border-l-emerald-500">
                <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Cash In (Dr)</div>
              ...
              <div className="bg-white px-4 py-3 border-r border-slate-300 border-l-4 border-l-rose-500">
                <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Cash Out (Cr)</div>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-slate-800 text-white">
                    ...
                    <th className="text-right px-3 py-2 font-semibold text-[10px] uppercase tracking-widest border-r border-slate-700">Cash In</th>
                    <th className="text-right px-3 py-2 font-semibold text-[10px] uppercase tracking-widest border-r border-slate-700">Cash Out</th>
                ...
                <tbody>
                  ...
                  {filtered.lines.length === 0 && (
                    <tr><td colSpan={8} className="text-center py-8 text-xs text-slate-400 uppercase tracking-widest">No cash transactions match filters</td></tr>
                  )}
                </tbody>
                <tfoot>
        ...
        {!cashAccountId && !loading && (
          <div className="min-h-[200px] flex items-center justify-center">
            <div className="text-xs text-slate-400 uppercase tracking-widest">Loading cash account...</div>
          </div>
        )}
      </div>
      ...
      {/* New Entry modal — pre-selected to cash account */}
      <NewJournalEntryModal
        open={newEntryOpen}
        accounts={accounts}
        preselectedAccountId={cashAccountId}
