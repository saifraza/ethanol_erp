  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-xs text-slate-400">Loading Payables Aging...</div>
      </div>
    );
  }
      ...
      <div className="p-3 md:p-6 space-y-0">
        ...
        <div className="bg-slate-800 text-white px-4 py-2.5 -mx-3 md:-mx-6 -mt-3 md:-mt-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h1 className="text-sm font-bold tracking-wide uppercase">Payables Aging</h1>
            <span className="text-[10px] text-slate-400">|</span>
            <span className="text-[10px] text-slate-400">Accounts Payable Analysis</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-0 border-x border-b border-slate-300 -mx-3 md:-mx-6">
          <div className="bg-white px-4 py-3 border-r border-slate-300">
            <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-0.5">Total Payable</div>
            <div className="text-xl font-bold text-slate-800 font-mono tabular-nums">{fmtCurrency(totalPayable)}</div>
          </div>
          <div className="bg-white px-4 py-3 border-r border-slate-300">
        ...
        <div className="border-x border-b border-slate-300 -mx-3 md:-mx-6 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody>
                {data.rows.map(row => (
                  <tr key={row.vendorId} className="border-t border-slate-200 hover:bg-blue-50/30 even:bg-slate-50/50">
                    <td className="px-3 py-1.5 font-medium text-slate-800 border-r border-slate-100">{row.vendorName}</td>
                ...
                {data.rows.length === 0 && (
                  <tr>
                    <td colSpan={6} className="px-3 py-8 text-center text-slate-400 text-xs">No payables data</td>
                  </tr>
                )}
              </tbody>
              {data.rows.length > 0 && (
