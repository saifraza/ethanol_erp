...
export default function PayablesAging() {
  ...
  if (loading) return <div className="p-6 text-gray-500">Loading Payables Aging...</div>;
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-bold text-gray-800">Payables Aging</h1>

      {/* KPI Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white rounded-lg shadow p-4">
          <div className="text-xs text-gray-500 uppercase tracking-wide">Total Payable</div>
          <div className="text-2xl font-bold text-gray-800 mt-1">{fmtCurrency(totalPayable)}</div>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
      ...
      <div className="bg-white rounded-lg shadow overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            ...
            {data.rows.length === 0 && (
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-gray-400">No payables data</td>
              </tr>
            )}
          </tbody>
          {data.rows.length > 0 && (
