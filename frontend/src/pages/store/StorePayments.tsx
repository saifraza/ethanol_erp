...
// One row per Store-category PO, served by GET /api/fuel/payments?category=...
// Same response shape as the fuel page; we declare a parallel interface here
// rather than refactoring FuelManagement (Phase 5 work).
interface StorePaymentRow {
  id: string;
  poNo: number;
  poDate: string;
  status: string;
  ...
  fuelName: string;   // backend reuses the field name; for store rows this is the line description / material name
  fuelUnit: string;
...
// Single-select chip filter. "ALL" re-includes the full Store category set.
type CategoryChip = 'ALL' | 'RAW_MATERIAL' | 'CHEMICAL' | 'PACKING' | 'SPARE' | 'CONSUMABLE' | 'GENERAL';
...
export default function StorePayments() {
  const [payments, setPayments] = useState<StorePaymentRow[]>([]);
  ...
  // The category list we send to the server. ALL chip = the full Store set;
  // any other chip narrows to a single category. The server upload-guard
  // accepts the same comma-separated form.
  const activeCategoryParam = categoryChip === 'ALL' ? ALL_STORE_CATEGORIES : categoryChip;

  const fetchPayments = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get<StorePaymentRow[]>('/fuel/payments', {
  ...
  const filtered = useMemo(() => {
    ...
    const matchesSearch = (p: StorePaymentRow) => {
      if (!q) return true;
      return (
    ...
    const passesStatus = (p: StorePaymentRow) => {
  ...
  // Chip counts — quick visual cue without a second fetch. Computed off the
  // current dataset, so they reflect the active server-side category param.
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="p-3 md:p-6 space-y-0">
        ...
        <div className="bg-slate-800 text-white px-4 py-2.5 -mx-3 md:-mx-6 -mt-3 md:-mt-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h1 className="text-sm font-bold tracking-wide uppercase">Store Payments</h1>
            <span className="text-[10px] text-slate-400">|</span>
            <span className="text-[10px] text-slate-400">Raw Material / Chemical / Packing / Spares / Consumables / General</span>
          </div>
          <button
        ...
        {/* Category chip row */}
        <div className="bg-white border-x border-b border-slate-300 -mx-3 md:-mx-6 px-4 py-2 flex items-center gap-2 flex-wrap">
        ...
        <div className="grid grid-cols-5 gap-0 border-x border-b border-slate-300 -mx-3 md:-mx-6">
          <div className="bg-white px-4 py-3 border-r border-slate-300 border-l-4 border-l-blue-500">
            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Store POs</div>
          ...
          {loading && payments.length === 0 ? (
            ...
            <div className="px-3 py-8 text-center text-xs text-slate-400 uppercase tracking-widest">
              {statusFilter === 'outstanding'
                ? 'No outstanding store payments — all caught up.'
                : statusFilter === 'paid'
                  ? 'No fully-paid store POs yet.'
                  : 'No store POs found for this category.'}
            </div>
          ) : (
      ...
      {/* Invoice list modal — InvoiceList handles uploads itself when
          onTriggerUpload is omitted (built-in basic multi-file POST). The
          server upload-guard accepts any of the Store-category lines. */}
      {invoicesModal && (
        <InvoiceList
          key={`store-invoices-${invoicesModal.poId}-${invoiceListNonce}`}
          poId={invoicesModal.poId}
          poNo={invoicesModal.poNo}
          vendorName={invoicesModal.vendorName}
          fmtCurrency={fmtCurrency}
          categories={ALL_STORE_CATEGORIES}
      ...
      {/* Pay dialog — surface=generic so it routes through the full
          /purchase-orders/:id/pay validation (cash-voucher, auto-close, etc.)
          rather than the fuel-specific endpoint. */}
      {payDialog && (
        <PayDialog
          poId={payDialog.poId}
          poNo={payDialog.poNo}
