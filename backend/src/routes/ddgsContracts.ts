...
router.get('/', asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const rows = await prisma.dDGSContract.findMany({
    ...
    select: {
      ...
      dealType: true, productType: true, processingChargePerMT: true, principalName: true,
      quantityType: true,
...
router.post('/', asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const contract = await prisma.dDGSContract.create({
    data: {
      ...
      productType: ['DDGS', 'WGS', 'MASH_BIOFUEL'].includes(b.productType) ? b.productType : 'DDGS',
...
router.put('/:id', asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const contract = await prisma.dDGSContract.update({
    ...
    data: {
      ...
      productType: ['DDGS', 'WGS', 'MASH_BIOFUEL'].includes(b.productType) ? b.productType : ((existing as any).productType || 'DDGS'),
