...
const AUDITED_FIELDS = ['nature', 'rate', 'threshold', 'effectiveFrom', 'effectiveTill', 'isActive'];

router.get('/', asyncHandler(async (req: AuthRequest, res: Response) => {
  const where: Record<string, unknown> = {};
  if (req.query.activeOnly !== 'false') where.isActive = true;

  const items = await prisma.tcsSection.findMany({
    where,
    orderBy: { code: 'asc' },
    take: 200,
    select: {
      id: true, code: true, nature: true, rate: true, threshold: true,
...
router.get('/:id', asyncHandler(async (req: AuthRequest, res: Response) => {
  const item = await prisma.tcsSection.findUnique({ where: { id: req.params.id } });
  if (!item) throw new NotFoundError('TcsSection', req.params.id);
  res.json(item);
}));
...
router.post('/', authorize('ADMIN', 'SUPER_ADMIN'), asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const existing = await prisma.tcsSection.findUnique({ where: { code: data.code } });
  if (existing) throw new ConflictError(`TCS section ${data.code} already exists`);

  const item = await prisma.tcsSection.create({
    data: {
      code: data.code,
      nature: data.nature,
      rate: data.rate,
...
router.put('/:id', authorize('ADMIN', 'SUPER_ADMIN'), asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const existing = await prisma.tcsSection.findUnique({ where: { id: req.params.id } });
  if (!existing) throw new NotFoundError('TcsSection', req.params.id);
  ...
  const updated = await prisma.tcsSection.update({ where: { id: req.params.id }, data });
