...
// ── GET /:id/delivery-challan-pdf ── HTML-based PDF
router.get('/:id/delivery-challan-pdf', asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const gstValue = Math.round(productValue * 0.05);
  const totalValue = productValue + gstValue;
  const fmtDate = truck.date.toLocaleDateString('en-IN', { day: '2-digit', month: '2-digit', year: 'numeric' });
...
<table>
<tr><th>Sno.</th><th>Description of Goods</th><th>Quantity</th><th>Unit</th><th>Rate (Rs.)</th><th class="right">Value (inc.5% GST)</th><th>Remarks</th></tr>
<tr><td>1</td><td>Ethanol</td><td class="right">${truck.quantityBL.toLocaleString('en-IN')}</td><td>Litres</td><td class="right">${productRate}/ltr</td><td class="right">${totalValue.toLocaleString('en-IN')}/-</td><td>(produced on job work)</td></tr>
</table>
...
<li>- Place of Supply: ${truck.destination || 'Odisha'}</li>
...
// ── GET /:id/gate-pass-pdf ── HTML-based PDF
...
<div class="sub">ETHANOL DISPATCH — ${truck.contract?.contractType === 'JOB_WORK' ? 'JOB WORK' : 'SALE'}</div>
...
<tr><td>${truck.contract?.contractType === 'JOB_WORK' ? 'Job Work Charges for Ethanol Production' : 'Ethanol'}</td><td>998842</td><td class="right">${truck.quantityBL.toLocaleString('en-IN')}</td><td>BL</td><td class="right">${rate}</td><td class="right">${amount.toLocaleString('en-IN')}</td></tr>
</table>
<table>
<tr><th>Gross (KG)</th><th>Tare (KG)</th><th>Net (KG)</th><th>Volume (BL)</th><th>Strength %</th></tr>
<tr><td class="right">${(truck.weightGross || 0).toLocaleString('en-IN')}</td><td class="right">${(truck.weightTare || 0).toLocaleString('en-IN')}</td><td class="right">${(truck.weightNet || 0).toLocaleString('en-IN')}</td><td class="right">${truck.quantityBL.toLocaleString('en-IN')}</td><td class="right">${truck.strength || '-'}</td></tr>
</table>
<div class="sig">
