...
/** Resolve account codes to IDs (cached per call). Scoped to companyId when provided. */
async function resolveAccounts(prisma: PrismaClient, codes: string[], companyId?: string): Promise<AccountCache> {
...
async function createJournalEntry(
  ...
  const entry = await tx.journalEntry.create({
    data: {
      ...
      companyId: params.companyId || MSPIL_COMPANY_ID,
      lines: {
...
export async function onPurchaseBooked(
  ...
  try {
    ...
    const accts = await resolveAccounts(prisma, allCodes, purchase.companyId);
...
export async function onStockMovement(
  ...
  try {
    ...
    const accts = await resolveAccounts(prisma, [ACCT.INVENTORY, ACCT.COGS, ACCT.TRADE_PAYABLE, ACCT.OTHER_EXPENSE], movement.companyId);
...
export async function onContractorBillConfirmed(
  ...
  try {
    ...
    const accts = await resolveAccounts(prisma, allCodes, bill.companyId);
...
// ═══════════════════════════════════════════════════════
// REVERSE an auto-generated entry — used when a source document
// (e.g. a sale invoice) is cancelled/voided. Finds the live (non-reversed)
// entry for refType+refId, marks it reversed, and posts a swapped entry so
// the ledger nets to zero. Idempotent: returns null if nothing to reverse.
// ═══════════════════════════════════════════════════════
export async function reverseAutoJournal(
  prisma: PrismaClient,
  refType: string,
  refId: string,
  userId: string
  ...
  try {
    ...
    if (!original) {
      console.warn(`[AutoJournal] No live entry to reverse for ${refType}/${refId}`);
      return null;
    }
    ...
    console.error(`[AutoJournal] Failed to reverse ${refType}/${refId}:`, err);
    return null;
  }
}
