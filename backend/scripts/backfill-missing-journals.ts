...
async function backfillSaleInvoiceNoJe() {
  ...
  const all = await prisma.invoice.findMany({
    ...
    select: {
      id: true, invoiceNo: true, totalAmount: true, amount: true,
  ...
  for (const inv of missing) {
    try {
      const jeId = await onSaleInvoiceCreated(prisma, {
        id: inv.id, invoiceNo: inv.invoiceNo, totalAmount: inv.totalAmount,
...
async function adjustTcsGap() {
  ...
  const tcsInvs = await prisma.invoice.findMany({
    ...
    select: {
      id: true, invoiceNo: true, totalAmount: true, tcsAmount: true, tcsSection: true, tcsPercent: true,
    ...
    try {
      ...
      await prisma.journalEntry.create({
        data: {
          ...
          narration: `INV-${g.inv.invoiceNo} ${g.inv.tcsSection || '206C(1)'} TCS adjustment`,
          refType: 'SALE',
          refId: g.inv.id,
          userId: g.inv.userId,
          ...
          lines: {
            create: [
              { accountId: a['1100'], debit: g.gap, credit: 0, narration: `TCS gap on INV-${g.inv.invoiceNo}`, division: 'ETHANOL' },
