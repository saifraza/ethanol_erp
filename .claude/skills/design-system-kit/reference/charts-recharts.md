...
> This is the design-system-kit-local mirror of the repo's `.claude/skills/design-system-kit/reference/charts-recharts.md` (Recharts chart design system). Same rules and code, faithful copy. The stale per-module audit/status table is not reproduced here — the OPC Live page (`OPCTagManager.tsx`) remains the reference implementation.
