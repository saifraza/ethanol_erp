---
name: uploads-s3-mirror
description: Dual-write upload pattern for the MSPIL ERP — every multer route mirrors files to the neat-shelf S3 bucket in real time so a Railway volume loss never loses customer files. Use when adding or editing a multer upload route, touching mirrorToS3, changing the uploadServe static-file path, debugging the Storage Health dashboard, or recovering files via aws s3 sync. Covers s3Storage.ts, uploadServe.ts, uploadBackupJob.ts, uploadBackupAdmin.ts, and StorageHealth.tsx.
when_to_use: Triggers on upload, multer, mirrorToS3, s3, bucket, neat-shelf, storage health, upload backup, disk-to-S3 fallback, or recovering a wiped Railway volume.
---
...
- **NEVER** write files outside `/app/backend/uploads` — the mirror only watches that path.
- **NEVER** `unlinkSync` an uploaded file before the response returns — `mirrorToS3` reads from disk after multer.
- **NEVER** call `mirrorToS3` for endpoints that *delete* files. Keep the bucket as a one-way archive; nothing in our flow deletes from it. Extra files in the bucket are harmless.
- **NEVER** change the relative path stored in the DB after a file is mirrored — the disk path and the bucket key share that path verbatim.
- **ALWAYS** save the relative path (not the absolute one) on the DB row.
- **ALWAYS** read files through the `uploadServe` route or `getFromS3` helper — never `fs.readFile` directly from route code, or you skip the S3 fallback and serve a 404 if the volume is fresh.
