...
>
> **Origin**: distilled from a sugar-WB anti-cheat brainstorm. Ethanol WB is the proving ground / training-data source. Sugar WB is the eventual enforcement target.

This is the design reference for the `wb-vision` skill. For the live action (pulling/viewing training events from the factory server), see `SKILL.md` in this directory.
...
> Note: a `vision/` directory like this is **gitignored** at repo root — never push ML weights, `.venv`, or training data to origin (root `CLAUDE.md` → "Local-only directories").
