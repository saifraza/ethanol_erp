...
# Stream stdout+stderr to the log
& $venvPython -m uvicorn bridge:app --host 0.0.0.0 --port 5005 *>> $logFile

Add-Content -Path $logFile -Value "$(Get-Date -Format o) uvicorn exited with code $LASTEXITCODE"
exit $LASTEXITCODE
